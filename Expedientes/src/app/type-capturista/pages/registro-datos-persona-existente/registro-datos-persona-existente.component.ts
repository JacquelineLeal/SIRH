...
@Component({
  selector: 'app-registro-datos-persona-existente',
  templateUrl: './registro-datos-persona-existente.component.html',
  styleUrls: ['./registro-datos-persona-existente.component.scss']
})
export class RegistroDatosPersonaExistenteComponent implements OnInit {
  ...
  async getListaEscolaridades(CVE_EMPLEADO: any){
    ...
    await this.datosEscolaresService.GetDatosEscolaresByCveEm(CVE_EMPLEADO).subscribe(
      res=>{
        ...
        var listaEscolaridadLenght = Object.keys(this.listaEscolaridad).length;
        console.log(this.listaEscolaridad);

        if(listaEscolaridadLenght == 0){
          this.newRegistrarDatosEscolares.CONSECUTIVO = 1;
         // console.log('concen true', this.newRegisDocumentos.CONSECUTIVO);

        }else{
          this.newRegistrarDatosEscolares.CONSECUTIVO = this.listaEscolaridad[0].CONSECUTIVO + 1;
          console.log('concen else', this.newRegistrarDatosEscolares.CONSECUTIVO);
        }
  ...
  async getListaIdiomas(CVE_EMPLEADO: any){
    ...
    await this.datosIdiomasService.getIdiomasByCveEmp(CVE_EMPLEADO).subscribe(
      res=>{
        ...
        var listaIdiomasLenght = Object.keys(this.listaIdiomas).length;
        
        console.log(this.listaIdiomas);

        if(listaIdiomasLenght == 0){
          this.newRegistrarIdiomas.CONSECUTIVO = 1;
         // console.log('concen true', this.newRegisDocumentos.CONSECUTIVO);

        }else{
          this.newRegistrarIdiomas.CONSECUTIVO = this.listaIdiomas[0].CONSECUTIVO + 1;
    ...
    await this.datosDocumentosService.getListaDocsByCveEmp(CVE_EMPLEADO).subscribe(
      res=>{
        ...
        if(lenghtListDocs == 0){
          ...
          this.newRegisDocumentos.CONSECUTIVO = this.listaDocsByCveEmp[0].CONSECUTIVO + 1;
          console.log('concen else', this.newRegisDocumentos.CONSECUTIVO);
        }
